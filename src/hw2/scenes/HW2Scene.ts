...
import PlayerController, { PlayerAnimations } from "../ai/PlayerController";
...
import BubbleBehavior from "../ai/BubbleBehavior";
...
export default class HW2Scene extends Scene {
	...
	public override updateScene(deltaT: number){
		...
		this.handleAirChange(MathUtils.clamp(currentAir - deltaT, 0, 20),20);
		...
		if (currentAir <= minAir) {
			this.handleHealthChange(MathUtils.clamp(currentHealth - deltaT*2, minHealth, maxHealth),maxHealth);
		}
	...
	protected spawnBubble(): void {
		...
		if (bubble) {
			...
			bubble.position.copy(RandUtils.randVec(this.viewport.getOrigin().x, viewportSize.x, viewportSize.y, paddedViewportSize.y));
	...
	public handleBubblePlayerCollisions(): number {
		...
		for (let bubble of this.bubbles) {
			let coll = new Circle(bubble.position,25);
			if (bubble.visible 
	...
	public handleMinePlayerCollisions(): number {
		...
		for (let mine of this.mines) {
			if (mine.visible 
				...
				mine.animation.playIfNotAlready(MineAnimations.EXPLODING);
				setTimeout(() => {
					mine.visible = false;
				}, 250);
				if (this.player.isCollidable) {
					...
					this.handleHealthChange(
						this.healthBar.size.x - this.healthBarBg.size.x * 1/10,
					this.healthBarBg.size.x);
					...
					this.player.animation.playIfNotAlready(PlayerAnimations.HIT);
					setTimeout(() => {
						this.player.animation.playIfNotAlready(PlayerAnimations.IDLE);
					}, 500);
				}
			}
		}	
